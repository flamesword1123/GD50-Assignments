...
-- value set to either true or false in Paddle Select State
-- addLocked = false
...
function LevelMaker.createMap(level)
    ...
    -- tier and color for spawning locked brick
    local lockedTier = 7
    local lockedColor = 5
    ...
    -- keep track of if locked brick has already been added
    local haveAddedLocked = false
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- if a locked block is called for and has not yet been added, then at every locatiom
            -- there is a ten percent chance it will spawn
            if addLocked and not haveAddedLocked and math.random(1, 10) == 1 and true or false then
                b.color = lockedColor
                b.tier = lockedTier
                b.locked = true

                haveAddedLocked = true
...
function setAddLock()
    addLocked = math.random(1,2) == 1 and true or false
end

function LevelMaker.getAddLock()
    return addLocked
end
