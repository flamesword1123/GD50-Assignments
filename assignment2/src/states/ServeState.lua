...
function ServeState:enter(params)
    ...
    -- bolean used to update paddle size only once
    self.hasUpdatedPaddle = false
...
function ServeState:update(dt)
    ...
    if self.hasUpdatedPaddle == false then
        if self.health < 3 and self.paddle.size > 1 then
            self.paddle.size = self.paddle.size - 1
            self.hasUpdatedPaddle = true
        end
    end
